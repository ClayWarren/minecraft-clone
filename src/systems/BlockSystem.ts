...
export class BlockSystem extends System {
  ...
  private selectedHotbarSlot: number = 0 // Default to first hotbar slot

  private textureLoader: THREE.TextureLoader
  private textures: Map<string, THREE.Texture> = new Map()
  private materials: Map<string, THREE.MeshLambertMaterial | THREE.MeshLambertMaterial[]> = new Map()
  ...
  private loadBreakingTextures(): void {
    for (let i = 0; i < 10; i++) {
      this.textureLoader.load(`/textures/breaking/breaking_${i}.png`, (texture) => {
  ...
  private loadTextures(): void {
    const blockTextureMap: Record<string, string | { all?: string; top?: string; bottom?: string; side?: string }> = {
      grass: { top: '/textures/blocks/grass_top.png', bottom: '/textures/blocks/dirt.png', side: '/textures/blocks/grass_side.png' },
      dirt: '/textures/blocks/dirt.png',
      stone: '/textures/blocks/stone.png',
      cobblestone: '/textures/blocks/cobblestone.png',
      wood: { top: '/textures/blocks/log_top.png', bottom: '/textures/blocks/log_top.png', side: '/textures/blocks/log_side.png' },
      ...
      crafting_table: { top: '/textures/blocks/crafting_table_top.png', side: '/textures/blocks/crafting_table_side.png', bottom: '/textures/blocks/planks.png' }
    }
    ...
    for (const blockId in blockTextureMap) {
      ...
      if (typeof textureInfo === 'string') {
        ...
        this.textureLoader.load(textureInfo, (texture) => {
        ...
        const faceOrder = [faceTextures.side, faceTextures.side, faceTextures.top, faceTextures.bottom, faceTextures.side, faceTextures.side]

        faceOrder.forEach((texPath, index) => {
          if (texPath) {
            texturePromises.push(new Promise((resolve, reject) => {
              this.textureLoader.load(texPath, (texture) => {
            ...
            const blockData = getBlockType(blockId)
            loadedMaterials[index] = new THREE.MeshLambertMaterial({ color: blockData ? blockData.color : 0x808080 })
            texturePromises.push(Promise.resolve(new THREE.Texture())) // Resolve with dummy texture
          }
        })

        Promise.all(texturePromises).then(() => {
          this.materials.set(blockId, loadedMaterials as any) // Store array of materials
  ...
  private breakBlock(position: BlockPosition): void {
    ...
    const ws = (window as any).game.ws // Access WebSocket from global game instance
  ...
  private createBlockMesh(position: BlockPosition, blockType: string): void {
    ...
    const finalMaterial = Array.isArray(material) ? material : [material, material, material, material, material, material] // Apply same material to all faces if not multi-face
    ...
    if (targetBlock) {
      ...
      if (blockMesh) {
        if (this.isMouseDown && this.currentTargetBlock &&
          ...
          if (blockMesh.userData.originalColor && (Array.isArray(blockMesh.material) ? (blockMesh.material[0] as THREE.MeshLambertMaterial).color.getHex() : (blockMesh.material as THREE.MeshLambertMaterial).color.getHex()) !== blockMesh.userData.originalColor) {
            if (Array.isArray(blockMesh.material)) {
              (blockMesh.material[0] as THREE.MeshLambertMaterial).color.setHex(blockMesh.userData.originalColor)
            } else {
              (blockMesh.material as THREE.MeshLambertMaterial).color.setHex(blockMesh.userData.originalColor)
            }
          }
        } else {
      ...
      if (lastMinedBlockMesh && lastMinedBlockMesh.userData.originalColor) {
        if (Array.isArray(lastMinedBlockMesh.material)) {
          (lastMinedBlockMesh.material[0] as THREE.MeshLambertMaterial).color.setHex(lastMinedBlockMesh.userData.originalColor)
        } else {
          (lastMinedBlockMesh.material as THREE.MeshLambertMaterial).color.setHex(lastMinedBlockMesh.userData.originalColor)
        }
      }
    }
  }
