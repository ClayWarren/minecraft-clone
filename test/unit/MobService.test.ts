...
describe('MobService', () => {
  ...
  describe('handleAttack', () => {
    ...
    it('should give loot to player when mob dies', () => {
      ...
      const hasLoot = mockPlayer.inventory.some(
        stack => stack && ['rotten_flesh', 'iron_ingot'].includes(stack.item.id)
      )
    ...
    it('should handle different mob types with different loot', () => {
      ...
      const sheepMob = Array.from(mobService.getAllMobs().values()).find(
        mob => mob.type === 'sheep'
      )
      ...
      const hasSheepLoot = mockPlayer.inventory.some(
        stack => stack && ['mutton', 'wool'].includes(stack.item.id)
      )
